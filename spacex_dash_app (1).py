...
def get_pie_chart(entered_site):
    ...
    pie_names=[]
    for elem in filtered_df['class']:
        if elem==0:
            pie_names.append('Failure')
        else:
            pie_names.append('Success')
    filtered_df['pie']=pie_names
    if entered_site=='ALL':
        fig = px.pie(filtered_df, values='class', 
        names='Launch Site', 
        title='Percentage of succes rates of all launch sites')
        return fig
    else:
        filtered_df1=filtered_df[filtered_df['Launch Site']==entered_site]
        fig = px.pie(filtered_df1, names='pie',  
        title=f'Succes rate percentage in launch site {entered_site}')
        return fig
# TASK 4:
